...
def reconstruct_image_from_representation(config, representation_placeholder, video_placeholder, progress_bar):
    dump_path = os.path.join(config['output_img_dir'], ('c' if config['content_img_name'] else 's') + '_reconstruction_' + config['optimizer'])
    dump_path = os.path.join(dump_path, os.path.basename(config['content_img_name']).split('.')[0] if config['content_img_name'] else os.path.basename(config['style_img_name']).split('.')[0])
    dir_exists = os.path.isdir(dump_path)
    ...
    if dir_exists:
        display_optimized_image(dump_path, num_of_iterations[config['optimizer']], video_placeholder) 
    else:
        if config['optimizer'] == 'adam':
            ...
            for it in range(num_of_iterations[config['optimizer']]):
                ...
                with torch.no_grad():
                    ...
                    utils.save(optimizing_img, dump_path, config, it, num_of_iterations[config['optimizer']], progress_bar)
            ...
            def closure():
                ...
                with torch.no_grad():
                    ...
                    progress_bar.progress((cnt + 1) / num_of_iterations['lbfgs'])
        ...
        progress_bar.empty()
        # display_optimized_image(dump_path, num_of_iterations[config['optimizer']], video_placeholder)  # Pass video placeholder
